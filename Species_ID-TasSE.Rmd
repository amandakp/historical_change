...
pacman::p_load(tidyverse, dplyr, ggplot2, patchwork, vegan, lme4, emmeans, ggeffects, patchwork, scales, multcomp)
```
...
#LIVE-DEAD DISCORDANCE ANALYSIS
...
results_df <- live_matrix %>%
  select(Site_code, Site_location, Sieve_size_mm, Replicate, group_id) %>%
...
# ─────────────────────────────────────────────────────
# 📈 Test effects with linear models
# ─────────────────────────────────────────────────────

# Linear mixed model (Replicates are nested within sites)

#Fixed effects: test whether Site_location or sieve size significantly affect discordance.
...
results_df <- results_df %>%
  mutate(across(c(Site_code, Replicate, Site_location, Sieve_size_mm), as.factor))

# Fit the mixed model
lmm <- lmer(live_dead_dissimilarity ~ Site_location + Sieve_size_mm + Site_code +
...
# Marginal means for Site_location
emmeans(lmm, pairwise ~ Site_location)
...
plot_sieve <- ggplot() +
  geom_jitter(data = results_df,
              aes(x = Sieve_size_mm, y = live_dead_dissimilarity,
                  color = sieve_size_mm_num),
              size = 2, width = 0.2, alpha = 0.8) +
  geom_line(data = pred_sieve, aes(x = x, y = predicted, group = 1),
            color = "black", linewidth = 1.2) +
  geom_ribbon(data = pred_sieve, aes(x = x, ymin = conf.low, ymax = conf.high),
  ...
  geom_text(data = cld_sieve,
            ...
            color = "black", size = 4) +
  labs(title = "Predicted live-dead dissimilarity",
...
plot_location <- ggplot() +
  geom_jitter(data = results_df,
              aes(x = Site_location, y = live_dead_dissimilarity,
                  color = Site_location),
              size = 2, width = 0.2, alpha = 0.8) +
  geom_line(data = pred_location, aes(x = x, y = predicted, group = 1),
            color = "black", linewidth = 1.2) +
  geom_ribbon(data = pred_location, aes(x = x, ymin = conf.low, ymax = conf.high),
              alpha = 0.2, fill = "darkgrey") +
